...
public class LogTimeMenuController implements Initializable {
    ...
    private void logTimeMouseClicked(Activity activity, int i) {
    	...
    	try {
			hoursArg = Double.parseDouble(textFieldElementX.getText().trim());
		} catch (NumberFormatException e) {
			ProjectAppNavigator.appendToOutputTextArea("Invalid input number for hours worked. Has to be Double, ex. '3.5'.");
			return;
		}   	
    	try {
			ProjectAppNavigator.projectApp.getActiveUser().logActivityTime(activity, hoursArg, ProjectAppNavigator.projectApp.getDate());
			...
			ProjectAppNavigator.appendToOutputTextArea("Input has to be a possitive double type number and user has to join the activity before logging.");
			return;
		}
