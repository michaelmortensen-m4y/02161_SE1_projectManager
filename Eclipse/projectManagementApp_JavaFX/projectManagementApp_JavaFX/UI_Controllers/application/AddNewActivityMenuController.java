...
public class AddNewActivityMenuController implements Initializable {
    		...
    		for (String employeeInitials : employeesInitials) {
    			...
    			if (ProjectAppNavigator.projectApp.getEmployees().contains(employee)) {
    				if (employee.isAvailable(startDate, endDate)) {
    					...
    					ProjectAppNavigator.appendToOutputTextArea("User with initials \"" + employeeInitials + "\" is not available in the given time period.");
    				}
    			} else {
    				ProjectAppNavigator.appendToOutputTextArea("No user with initials \"" + employeeInitials + "\" exist.");
    			}
    ...
    void editActivityMouseClicked(MouseEvent event) {
		...
		if (choiceBoxValue.equals("Activity name")) {
			...
			Date newDateStart;
			try {
				newDateStart = newSdfStart.parse(editObjectTextValue.trim());
				startDateText = editObjectTextValue.trim();
			} catch (ParseException e) {
				ProjectAppNavigator.appendToOutputTextArea("No such activity has been added to the activity list.");
				return;
			}
			...
			Date newDateEnd;
			try {
				newDateEnd = newSdfEnd.parse(editObjectTextValue.trim());
				endDateText = editObjectTextValue.trim();
			} catch (ParseException e) {
				ProjectAppNavigator.appendToOutputTextArea("No such activity has been added to the activity list.");
				return;
			}
			...
			List<Employee> employees = new ArrayList<Employee>();
			if (!(editObjectTextValue.trim().equals("Assign employees (aaaa bbbb ...)") || editObjectTextValue.trim().isEmpty())) {
	    		String[] employeesInitials = editObjectTextValue.trim().split(" ");
	    		Employee employee;
	    		for (String employeeInitials : employeesInitials) {
	    			employee = ProjectAppNavigator.projectApp.getEmployeeByInitials(employeeInitials);
	    			if (ProjectAppNavigator.projectApp.getEmployees().contains(employee)) {
			...
			System.out.println("'" + choiceBoxValue + "' and '" + editObjectTextValue + "' gave an error.");
			editObjectText.setText("New value");
		}
		System.out.println(startDate.WEEK_OF_YEAR);
		addedActivitiesTextArea.setText(addedActivitiesTextAreaContent.toString().replace("- " + activityField.getText().trim() + " | " + budgetedTimeField.getText().trim() + " | " + startDateField.getText().trim() + " | " + endDateField.getText().trim() + " | " + employeesField.getText().trim() + "\n", "- " + activityName + " | " + budgetedTime + " | " + startDateText + " | " + endDateText + " | " + emplString + "\n"));
    	addedActivitiesTextArea.setScrollTop(Double.MAX_VALUE);
		
		ProjectAppNavigator.appendToOutputTextArea("Edited activity succesfully.");
