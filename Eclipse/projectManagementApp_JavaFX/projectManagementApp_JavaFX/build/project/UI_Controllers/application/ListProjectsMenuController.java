...
public class ListProjectsMenuController {
    ...
    void searchByManagerMouseClicked(MouseEvent event) {
    	...
    	if (ProjectAppNavigator.projectApp.getEmployeeByInitials(managerInitialsField.getText()) == null) {
    		ProjectAppNavigator.appendToOutputTextArea("No employee with initials '"+ managerInitialsField.getText() + "' has been registered.");
    		return;
    	}
    	Employee employee = ProjectAppNavigator.projectApp.getEmployeeByInitials(managerInitialsField.getText());
    	projectsTextAreaContent.setLength(0);
    	projectsTextArea.setText(projectsTextAreaContent.toString());
		projectsTextArea.setScrollTop(Double.MAX_VALUE);
    ...
    void editByIdMouseClicked(MouseEvent event) {
    	try {
    		ProjectAppNavigator.activeProject = ProjectAppNavigator.projectApp.getProjectById(Integer.parseInt(idField.getText()));
    	} catch (NumberFormatException e) {
    		ProjectAppNavigator.appendToOutputTextArea("Invalid serialnumber. Is has to be an integer.");
    		return;
    	}
    	if (ProjectAppNavigator.activeProject == null) {
    		ProjectAppNavigator.appendToOutputTextArea("No project with serialnumber '" + idField.getText() + "' exist.");
    		return;
    	}
    	if (!ProjectAppNavigator.activeProject.getProjectManager().equals(ProjectAppNavigator.projectApp.activeUser)) {
    		ProjectAppNavigator.appendToOutputTextArea("You are not the project manager for the project with ID '" + idField.getText() + "'.");
    		return;
    	}
